...
const loginType = ref(PASSWORD_LOGIN)  //0为密码登录，1为扫码快捷登录
...
async function* pollScanResult() {
  while (true) {
    ...
    // 判断是否满足退出条件
    if (await (exitDirect || checkCondition(loginStatus.QRAuthorityInfo))) {
...
async function checkCondition(QRAuthorityInfo) {
  return await loginStatus.checkScanResult(QRAuthorityInfo)
}

const qrLoginChecker = pollScanResult();

async function startChecking() {
  let nextCheck = await qrLoginChecker.next();

  while (!nextCheck.done) {
    await nextCheck.value; // 等待 Promise 解析
    nextCheck = await qrLoginChecker.next(); // 继续下一个迭代
  }
...
const quickLogin = async (type) => {
  exitDirect = false
  loginStatus.scanPrompt = '请扫描二维码'
  loginType.value = type
  await loginStatus.fetchQRImg()
  startChecking();
}
...
const debounceRefresh = debounce(refreshQR, 500)
const exitLogin = () => {
  exitDirect = true
  ...
  <div class="fullview-container">
    <div class="login-container">
      <div class="login-head">
        ...
        <span class="closed-login iconfont icon-chacha" @click="exitLogin"></span>
      </div>

      <div class="login-main">
        <div class="left-main">
          <div class="toggle-parcel" v-if="loginType === PASSWORD_LOGIN">
            <h2 class="password-login">密码登录</h2>
            <p class="quick-login">推荐使用<a class="quick" @click="quickLogin(QUICK_LOGIN)">快捷登录</a>,防止盗号</p>
            <div class="login-input-box">
              <div class="enter-password-box" v-if="phonenumVertifyStep === 1">
                <input class="diy-input" type="text" name="" id="" placeholder="支持QQ号/邮箱/手机号登录" v-model="account"
                  @focus="clearMsg" @blur="judgeType">
                <input class="diy-input" type="password" placeholder="请输入密码" v-model="password" @focus="clearMsg"
                  @blur="judgeType">
              </div>
              ...
              <div class="vertification-container" v-if="phonenumVertifyStep === 2">
                <input type="text" class="diy-input vertification" placeholder="请输入验证码" v-model="vertificationCode"
                  @focus="clearMsg">
              ...
              <div class="error-occupation">
                <p class="error-tips" v-if="loginStatus.errorMessage !== null"><span
                    class="iconfont icon-tixingshixin"></span>{{ loginStatus.errorMessage }}</p>
                <p class="success-tips" v-if="loginStatus.successMessage !== null"><span
                    class="iconfont icon-shixingouxuan"></span>{{ loginStatus.successMessage }}</p>
              </div>

              <div class="login-btn" @click="login">登录</div>
            </div>
          </div>
          <div class="toggle-parcel" v-if="loginType === QUICK_LOGIN">
            ...
            <div class="login-scan">
              <div class="scan-container">
                <div class="QR" @click="debounceRefresh">
                  <img :src="loginStatus.QRImg" alt="">
                </div>
                <!-- 头像点击登录，待做 -->
              ...
              <p>{{ loginStatus.scanPrompt }}</p>
            </div>
          </div>

          <div class="login-options">
            <a target="_blank" href="https://accounts.qq.com/find/password" class="public"
              v-if="loginType === PASSWORD_LOGIN">找回密码</a>
            <a href="javascript:;" class="public" v-if="loginType === QUICK_LOGIN"
              @click="loginType = PASSWORD_LOGIN">密码登录</a>
            <a target="_blank" href="https://ssl.zc.qq.com/v3/index-chs.html?from=pt" class="public">注册账号</a>
            <a target="_blank" href="https://support.qq.com/products/77942?customInfo=.appid100497308"
              class="public">意见反馈</a>
          </div>
        </div>
        <div class="right-main">
          ...
          <p class="selected-box">授权即同意<a target="_blank"
              href="https://rule.tencent.com/rule/preview/46a15f24-e42c-4cb6-a308-2347139b1201">服务协议</a>和<a
              target="_blank"
              href="https://rule.tencent.com/rule/preview/3fd52bde-6555-453b-9ab8-c5f1f3d22c62">QQ隐私保护指引</a></p>
