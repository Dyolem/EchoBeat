<template>
    ...
    <div class="playlist-item" @mouseover="showMask" @mouseleave="hideMask">
        <play-cover :jump-id="baseInfo.tid" :cover="cover" :active-width-and-height="{width:240,height:240}" @passPlaySignal="jumpToBeplay" :allowDetail="true"></play-cover>    
         <div class="introduce" >
             <div class="introhead">
                
                 <router-link :to="{name:'songlist-details',params:{dissid:baseInfo.tid}}" >{{ baseInfo.title }}</router-link>
             </div>
            <div class="play-volumn">
                <!-- creator根据需要选择是否插入 -->
                <div class="creator">
                    <slot name="creator" :creator="creatorInfo"></slot>
                </div>
...
const songListDetailStore = useSongListDetailStore()
...
//注意如果不是在模板中使用传过来的数据，需要加上props
    const data=ref(props.baseInfo.playNum)
    //控制小数位数显示
