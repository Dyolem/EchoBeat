    ...
    <div class="newsong-container">
        <div class="newsong-wheel" ref="newsongWheelRef">
            <div class="unit" v-for="(broadItem,broadIndex) in renderCount" :key="broadIndex">
                <div class="newsong-item" v-for="(item,index) in newSong.songlist.slice(broadIndex*9,(broadIndex+1)*9)" :key="index">
                    <div class="song-info">
                        <div class="play-cover-box" @click="jumpToPlay(index+broadIndex*9)">
                            <play-cover :jumpId="item.mid" :cover="coverUrlFormatted(item.album.mid)" :activeWidthAndHeight="{width:90,height:90}"
                                         :shringkButtonFlag="true" :allowDetail="false" >
                            </play-cover>
                        </div>
                        
                        <div class="info">
                            <h4 class="song-name"><router-link :to="{ name:'song-details',params:{mid:item.mid}}"> {{ item.title }}</router-link></h4>
                            <p class="singers-box">
