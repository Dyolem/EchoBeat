...
const storeRecentListenInfo = () => {
  recentClickSonglist.value =
    JSON.parse(localStorage.getItem("recentClickSonglistCategoy")) || []
  if (
    recentClickSonglist.value.length == 4 &&
    recentClickSonglist.value[recentClickSonglist.value.length - 1].id !=
      recentListnenInfo.value.id
  )
  ...
  localStorage.setItem(
    "recentClickSonglistCategoy",
    JSON.stringify(recentClickSonglist.value),
  )
}
// 当路由查询参数变化时请求数据
watch(
  ...
  { immediate: true },
) //这里必须使用immediate，因为组件重新挂载的时候，路由查询参数初始化完成后，监听器还未进行监听，所以不会请求数据。
...
<template>
  <playlist-recommendation
    :renderCount="Math.ceil(songlistCategoryStore.categoriesLength / 4)"
  >
    <template #wheel-container="slotProps">
      <wheel-catogories
        :travelDistance="slotProps.passTravelDistance"
      ></wheel-catogories>
    </template>
  </playlist-recommendation>
  ...
  <div class="classified-container">
    <div class="kindTitle">
      ...
      <span
        class="clear-current-kind iconfont icon-chacha"
        @click="clearCurrentKind"
      ></span>
    </div>
    <div class="playlists-container">
      <div
        class="playlist-item"
        v-for="(item, index) in songlistStore.songlist"
        :key="item.dissid"
      ...
      >
        ...
        <play-cover
          :jump-id="item.dissid"
          :cover="item.imgurl"
          :active-width-and-height="{ width: 240, height: 240 }"
          :allowDetail="true"
        >
        </play-cover>
        <div class="introduce">
          <div class="titletxt">
            <router-link
              ...
              :to="{
                ...
                params: { dissid: item.dissid },
              }"
              >{{ item.dissname }}</router-link
            >
          </div>
          <div class="creator">
            <router-link class="txt-color" to="">{{
              item.creator.name
            }}</router-link>
          </div>
