...
<template>
    <div class="more-kinds" v-if="categoryIndex!==-1?true:false" @click="prohibitPopover($event)">
        <div class="additional-kinds" v-for="(moreitems,idx) in songlistCategoryStore.categories[categoryIndex].list.slice(8,songlistCategoryStore.categories[categoryIndex].list.length)"
                ...
                <div class="wheelplay-container" ref="wheelplayContainerRef">
                    <div class="broad-category">
                        ...
                        <div class="kind-container">
                            <div class="kind" v-for="item in recentClickCategory" :key="item.id"
                            @click="fetchKindSonglist(item)">{{ item.name }}</div>
                        </div>
                    </div>
                    ...
                    就对应到了3号盒子，最终修改颜色是3号，因此需要保持索引不变，使用v-show -->
                            <div class="kind" v-show="item.list.length>8?true:false" @click="moreIndex(index,$event)" ref="moreRef">更多</div>
                        </div>
                    </div>
