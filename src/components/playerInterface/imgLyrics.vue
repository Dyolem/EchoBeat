...
watch(lyricsIndex, () => {
  ...
  // 如果未在拖动状态，则更新歌词状态
  if (!isDragging.value) midLyricsRef.value.scrollTop = lyricsIndex.value * 30
})
...
const vDrag = {
  ...
  mounted(el) {
    ...
    function handleDragStart(event) {
      ...
      miniSecond.value = 0
      if (waitDragging !== null) clearInterval(waitDragging)
  ...
  <div class="mod-song-info">
    <div class="song-info">
      <div class="mid-box">
        ...
        <div class="info">
          <div class="songname public" @click="toSongDetalisPage(lyrics.mid)">
            歌曲名：{{ songPlayLink.songName }}
          </div>
          <div class="singer public">
            歌手：<span v-for="item in songPlayLink.singerName"
              >{{ item.name }}&nbsp;</span
            >
          </div>
          <div class="album public">专辑：{{ songPlayLink.albumname }}</div>
        </div>
      </div>
    </div>
    ...
    <div class="mid-lyrics" ref="midLyricsRef" v-drag>
      <div
        class="lyrics-item"
        :class="lyricsIndex === index ? 'activeGreen' : ''"
        v-for="(item, index) in lyrics.songLyrics"
        :key="index"
      >
        {{ item.txt }}
      </div>
...
.img-container::after {
  ...
  background: url("../../assets/img/album_cover_player.8dfd80d6.png") 0 0
    no-repeat;
}
.song-img {
  width: 100%;
...
@media screen and (max-width: 450px) {
  ...
  .song-info {
    padding: var(--img-padding-y) 0;
    margin-bottom: 0;
  }

  .mid-lyrics {
    height: calc(100% - (300px + var(--img-padding-y) * 2));
  }
}
</style>
