...
function checkedSongs(index) {
  if (allCheckedRef.value.checked) allCheckedRef.value.checked = false
...
watch(
  [() => songPlayLink.playFlag, () => songPlayLink.playIndex],
  (newVal, oldVal) => {
...
// 点击歌曲后新建一个窗口展示歌曲详情页面
const directToSongDetailsPage = (mid, index) => {
  if (isMobile.value) {
    playsong(index)
    return
  }
...
async function playsong(index) {
  if (songPlayLink.playIndex !== index) {
    ...
    const result = await songPlayLink.fetchSongPlayLink(index)
    console.log(result)
    if (!result) {
      songPlayLink.$reset()
    ...
    await lyrics.fetchLyrics(
      JSON.parse(localStorage.getItem("toBePlayedSongs"))[index].mid,
    )
...
<template>
  <div class="songlist">
    ...
    <div
      class="song-bar"
      v-for="(item, index) in toBePlayed.song"
      :key="index"
    ...
    >
      ...
      <div class="song-name">
        <div class="song-occupation">
          <div
            ...
            ref="songNumRefs"
            :class="
              songPlayLink.playFlag && songPlayLink.playIndex === index
                ? 'song-number'
                : ''
            "
          >
          ...
          <div
            class="song-title"
            @click="directToSongDetailsPage(item.mid, index)"
          >
            {{ item.songname }}
          </div>
          <div class="MV MV-vip-public" v-if="item.mv.id !== 0">
            <router-link to="">MV</router-link>
          </div>
          <div class="vip MV-vip-public" v-if="item.pay !== 0">VIP</div>
          <div class="play-feature" v-if="activeIndex === index">
            <div class="play feature-public" @click="playsong(index)">
              <span
                class="iconfont"
                :class="
                  index === songPlayLink.playIndex && songPlayLink.playFlag
                    ? 'icon-ai07'
                    : 'icon-bofang'
                "
              ></span>
      ...
      <div class="singer">
        <span v-for="(i, singeridx) in item.singername" :key="singeridx"
          >{{ i.name }}&nbsp;</span
        >
      </div>
      <div class="time">
        <div class="feature-public">
          ...
          <span
            class="del-bucket iconfont icon-shanchu"
            @click="delBucket(index)"
...
/*注意这个样式是使用了的，只是通过动态类名来使用，IDE可能会显示为灰色*/
.song-number {
  width: 10px;
  height: 10px;
